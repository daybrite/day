...
//! (`IUIElement*`, via the shim's `day_xaml_unbox` seam), paired with the concrete native
...
//! C++/WinRT, compiled exactly the way `day-piece-picker` compiles its own XAML shim (see
//! `pieces/day-piece-picker/src/lib-xaml-shim.cpp` and the recipe in docs/tweaks.md):
//!
//! ```ignore
//! // src/my_tweak.cpp — link against the same WindowsApp.lib day-xaml-sys already links:
...
//! //   }
//! use day_xaml::XamlExt;
//! slider(v).xaml_raw(|abi, class| {
...
/// The XAML control class Day's shim (`day-xaml-sys`) realizes for `kind`. `""` for
