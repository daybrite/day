...
public final class DayBridge {
    ...
    // --- picker (docs/picker.md): style 0 = menu (Spinner), 1 = segmented (button row),
    //     2 = inline (RadioGroup). All report selection via nativeOnEvent kind 4 (SelectionChanged). ---
    public static View makePicker(final long id, int style, String joinedItems, int selected) {
        String[] items = joinedItems.isEmpty() ? new String[0] : joinedItems.split("\n");
        if (style == 0) {
            return makeSpinner(id, joinedItems, selected);
        } else if (style == 1) {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                b.setAllCaps(false);
                b.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View x) { selectSegment(row, idx); nativeOnEvent(id, 4, idx, null); }
                });
                row.addView(b);
            }
            ...
            RadioGroup g = new RadioGroup(ctx);
            for (int i = 0; i < items.length; i++) {
            ...
            g.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                public void onCheckedChanged(RadioGroup grp, int checkedId) {
                    if (checkedId > 0) nativeOnEvent(id, 4, checkedId - 1, null);
                }
            });
            return g;
        }
