...
than the Caption unit beside it. Centre those boxes and the text lands on three different
...
container — and a baseline-aligned row falls back to centring that child.
...
The size matters. A control that centres its text vertically moves its baseline with its height,
...
|---|---|---|
| AppKit | the control's font, centred in its `alignmentRectInsets` box (see below) | Native |
| GTK | `gtk_widget_measure`'s natural-baseline out-param | Native |
| Android | `View.getBaseline()` | Native |
| UIKit | derived: the view's font ascent, centred in its height | Emulated |
| Qt | derived: `QFontMetricsF::ascent()`, centred in its height | Emulated |
...
text placement can be wrong. `Unsupported` would mean rows fall back to centring, which is what
...
control's own font, centred in its alignment rect, baseline an ascender below the line's top. That
...
first line sits at the top of its text container rather than centred in it, and a control with no
...
would silently centre the very children the author asked to align, and the feature would look
...
measure a 0px spread between the label, the field's text and the trailing unit; centred, they
drift 3–4px apart.
