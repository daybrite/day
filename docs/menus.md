...
*model*; the backend owns the *rendering*, so a menu looks and behaves like a first-class citizen of
...
familiar items keep their native label, default shortcut, automatic enable/disable, and — crucially —
their **focus targeting**: Edit ▸ Copy copies from whatever control has focus, with no wiring:
...
equivalent on a platform simply render as an inert labelled item — nothing is imposed.
...
- **iOS/iPhone** — **no-op by design.** Touch platforms have no persistent global menu bar; the native
...
made from a menu coalesce into one update, exactly like a button tap). Standard roles carry no id — they
