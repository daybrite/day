...
> `QDoubleSpinBox` shim — and every other target gets the same composed row, built from
...
through day-core's `set_probe_value` seam — so `assert_text` sees the display form
...
(the stroke-width row of its inspector). There is no XAML or web arm on purpose — both
platforms have native spin controls (a WinUI `NumberBox`, `<input type="number">`), and arms
for them are welcome, but the composed row already behaves correctly there, so they are an
upgrade rather than a gap.
