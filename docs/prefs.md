...
> **Status: implemented** as `day-part-prefs` (in `parts/`, the headless counterpart of `pieces/`) — a
> **headless** day-ecosystem crate (no UI Piece): a shared cross-platform API for a small persistent
...
Values **persist across launches** — that is the whole point. There are **no features**: platform
selection is purely `#[cfg(target_os)]`, because persistence is an OS concern, not a widget-toolkit
one. `parts/day-part-prefs/examples/prefs.rs` is a plain `main` that uses it with no Day framework at
all (run it twice to watch a value survive the process).
...
- **macOS / iOS** — `NSUserDefaults.standard` is the system's per-application preferences store (a
  ...
  the only `unsafe` objc2 call (the value must be a property-list type — we always pass a real
...
Like `day-part-battery` and `day-part-network`, this is a headless external crate — no UI Piece,
nothing registered into any backend's `RENDERERS` slice. On Android it stages its own Java shim through
`[package.metadata.day.android]` (with **no** permission this time — private storage needs none), which
`day build` folds into the app's Gradle build with zero edits to any core day crate. On every other
platform it is fully day-independent (pure FFI on Apple, pure `std` file I/O on desktop). See
docs/extending.md.
