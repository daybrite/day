...
Honest reality (from §13's truth table): apple + Qt have first-class native a11y on all their OSes;
**GTK has no AT bridge on macOS** (works via AT-SPI on Linux); Android role/hint need an
`AccessibilityDelegate` (deferred). These are secondary combos for exactly this reason.
