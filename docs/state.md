...
almost the whole of "how do I structure an app's state":
...
bundle of handles: it moves into an event handler or a page function with no `Rc`, no `clone()`,
and no lifetime. That is what makes passing state around cost as little as reaching for a global.
...
exactly the case SwiftUI's `@EnvironmentObject` exists for. When you *can* pass it, pass it: an
argument is clearer than a lookup, and `Scene` is `Copy`.

For state that genuinely belongs to the whole app rather than to a window — a login session, a
...
building in*, and nothing more magical than that.
