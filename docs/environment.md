...
| `DAY_SIGN_STORE_PASS`, `DAY_SIGN_KEY_PASS` | The keystore and key passwords `day sign apply --keystore` signs with. They live here rather than in the argument list, which every other process on the machine can read, and they are what lets a distributor sign a package without the project that built it ([docs/packaging.md](https://daybrite.dev/docs/packaging/#signing-an-existing-package)) |
| `DAY_FLAVOR` | The build flavor to apply, the global `--flavor` flag from the environment: `day build`, every scripted launch and `day pack` read `Day-<name>.toml` as a layer over `Day.toml` ([docs/flavors.md](flavors.md)). One `env:` line builds a whole CI matrix leg as the flavor, and the value crosses into the xcodebuild and gradle callbacks so the identity they write is the flavor's |
