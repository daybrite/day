...
> registered link-time into each backend's renderer slice with **zero edits** to day. It wraps the
...
map()
    ...
    .frame(320.0, 240.0)          // a map is a growing leaf — constrain it
    .id("map")
```
...
also be **bound reactively** with `.center_signal(signal)` — a `Signal<(f64, f64)>` or a
...
- **AppKit** — `objc2-map-kit` binds `MKMapView` as an `NSView` subclass (macOS only). The region is
  ...
  **keyless** — no developer token or entitlement is needed for a plain map. The recenter patch uses
...
- **UIKit** — `objc2-map-kit` generates the `MKMapView` **struct for macOS only** (the WKWebView /
  ...
  + CoreLocation must be linked for the ObjC classes to register — declared via
  `[package.metadata.day.ios] frameworks = ["MapKit", "CoreLocation"]`, linked by the generated
...
- **GTK / Qt / Android / WinUI / mock** — the features exist (so an app can enable
  ...
  Maps SDK, `MapControl`), each with its own API-key and licensing story — out of scope for a small
  reference piece. This is the intentional "not every piece supports every platform" example: a
  Skip-style honest gap rather than a broken stub.
...
Apple backends; every other backend simply renders the placeholder leaf for the `day.piece.map` kind.
...
system frameworks the hand-rolled `MKMapView` needs — zero edits to any core day crate.

## Testing

The crate's smoke test boots the piece on the mock toolkit (which realizes unknown kinds as plain
widgets and ignores unknown patches — exactly like a backend built without the feature), moves the
