...
The finding that shaped the piece: Day targets eight toolkits, and **all eight ship a real styled
...
The offset column is the one that bites: a range is a **byte** range into a Rust `String`
...
editor edits it. One type, one vocabulary.
...
`space_after`. Paragraph attributes are deliberately **not** in `TextRun`: a paragraph attribute
applies to whole paragraphs, and folding it into a character run makes invalid states
representable.

Everything a toolbar needs is a pure function over that document — no controller, no synchronous
duty into the backend, identical on every target, and testable on the headless one:
...
loss next to the code that causes it. The headlines: Markdown has no syntax for color, background
or size, so those runs export as plain text; a heading scale that is not one of the six levels
exports as `**bold**`; the RTF reader is a **round-trip subset** — what Day writes, plus the
control words a word processor's plain paragraph uses — not a full RTF implementation.

## 3. Who owns the attributes

**Day does.** The native view owns the *characters* — typing, deletion, IME composition, undo,
autocorrect, dictation — and reports them as `Event::TextChanged`. The piece diffs that text
against the text it last knew, reflows its runs over the edit, and writes the bound signal.
...
The cost is stated plainly: **there is no attribute read-back.** Pasting styled text keeps its
...
- an edit that touched two separate places comes back as one span covering both — coarser, never
  wrong, and it costs only the styling between them.

`reflow` then moves the runs: ranges before the edit are untouched, ranges after shift, a range
containing the edit grows or shrinks, and an emptied range is dropped. O(runs), not O(document),
and no backend cooperation at all — which is what makes one code path serve all eight arms.
...
`SetAttributes` is the syntax-highlighting path, and the reason the piece is usable: re-tokenizing
produces fresh runs for the same characters on every keystroke, and pushing those as a document
would send the caret back to wherever the app last put it — and clear the undo stack with it. Every
...
The one piece of editor state an app cannot derive: with a collapsed caret there is no text to read
...
`allowsEditingTextAttributes = false` is the iOS-only line: it is what keeps the selection's edit
menu from offering B/I/U. Italic is a descriptor trait, asked for on top of the traits the font
already has, so bold+italic stays bold.

**gtk.** Tags, not attributes. Each distinct `RunStyle` becomes one interned `GtkTextTag`, keyed by
a canonical string and reused for the buffer's life — a fresh tag per run per keystroke would grow
the tag table without bound under a live highlighter. Offsets are characters. Two losses: Pango has
no dotted underline (it draws a single rule) and its wavy underline is the spell-check squiggle
(`Underline::Error`). Paragraph attributes ride the same tags, with the marker's hanging indent
spelled as a negative first-line offset against a wider left margin — the inverse of Apple's
spelling of the same layout.

**qt.** `QTextEdit`, driven through a `QTextCursor` in this crate's own shim
(`src/lib-qt-shim.cpp`). Not `setHtml`: that replaces the document, moving the caret and clearing
undo. `beginEditBlock`/`endEditBlock` collapse a sweep into one undo step and one relayout. Qt draws
no double underline (it degrades to single). Monospace comes from
`QFontDatabase::systemFont(FixedFont)` — Qt's rich text does not resolve the generic `monospace`
family from a char format, which is the same wall the label path hit.
...
`onSelectionChanged` is a protected method with no listener form. Losses: Android has one underline
span (dotted and wavy draw a plain rule), no per-paragraph justification, and no paragraph-spacing
span at all.
...
`Event::TextChanged`. Loss: ArkUI carries one decoration per span, so an underlined strikethrough
keeps the strikethrough.

**dom.** A `contenteditable` element — not a `<textarea>`, which is plain text by definition.
...
answer, and the difference is worth stating because the reasoning does not transfer. A color picker
...
character, word and line. A canvas with a blinking rectangle gets all of those wrong, and gets them
wrong invisibly — in the locales and for the users least able to work around it. Every platform
ships this control precisely because it is not reasonable to write.
...
4. **day-dom** grew the editor seam: `Dom::set_html` (caret-preserving), `Dom::set_editor_selection`,
   the `listen::EDITABLE` bit, and event kind 17 — the piece channel, as the Android and ArkTS
   bridges already spell it.
5. **day-mock** grew `MockProbe::describe_patch`, so a standalone piece's own patch type shows up in
   the op log instead of `?`. Without it, "this write patched attributes and did NOT replace the
   document" is not a thing a test can assert.
...
below it — still the right control for a chat composer or a commit message.
