...
// AutoSuggestBox (the WinUI search control: a text box with a query magnifier icon), boxed into a
// Day handle via the day_winui_box/day_winui_unbox seam that day-winui-sys exports, so this piece
// carries its own WinUI native code with ZERO edits to day's toolkit crates.
//
...
void *day_search_winui_new(const char *placeholder, const char *initial, uint64_t id,
                           void (*cb)(uint64_t, const char *)) {
    WUXC::AutoSuggestBox box;
    box.QueryIcon(WUXC::SymbolIcon(WUXC::Symbol::Find));
    box.PlaceholderText(hs(placeholder));
    if (initial && *initial)
        box.Text(hs(initial));
    box.TextChanged([id, cb](WUXC::AutoSuggestBox const &s,
                             WUXC::AutoSuggestBoxTextChangedEventArgs const &) {
        std::string t = to_utf8(s.Text());
        cb(id, t.c_str());
    });
    return day_winui_box(winrt::get_abi(box));
}

void day_search_winui_set_text(void *handle, const char *text) {
