...
void *day_combo_xaml_new(const char *items_joined, const char *text, const char *placeholder,
...
void day_combo_xaml_set_items(void *handle, const char *items_joined) {
...
void day_combo_xaml_set_text(void *handle, const char *text) {
    WUX::UIElement e{nullptr};
    winrt::copy_from_abi(e, day_xaml_unbox(handle));
    if (auto box = e.try_as<WUXC::ComboBox>()) {
        auto nt = hs(text);
