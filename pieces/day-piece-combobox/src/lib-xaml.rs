...
unsafe extern "C" {
    ...
    fn day_combo_xaml_set_items(w: *mut c_void, items_joined: *const c_char);
    fn day_combo_xaml_set_text(w: *mut c_void, text: *const c_char);
...
fn make(_backend: &mut Xaml, p: &ComboProps, id: NodeId) -> WinHandle {
    WinHandle(unsafe {
        day_combo_xaml_new(
...
fn update(_backend: &mut Xaml, h: &WinHandle, patch: &ComboPatch) {
    unsafe {
        match patch {
            ComboPatch::Items(items) => day_combo_xaml_set_items(h.0, joined(items).as_ptr()),
            ComboPatch::SetText(t) => day_combo_xaml_set_text(h.0, cstr(t).as_ptr()),
        }
    }
}
