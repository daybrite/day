...
//! and the commands. The seam between the two files is exactly one signal (`selected()`), which
//! is also the seam between the two layouts: the desktop mounts [`editor_pane`] beside the list,
//! the phone pushes [`editor`] over it, and neither side knows which happened.
