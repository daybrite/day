...
// the OpenHarmony public release material bundled in the SDK, with no Huawei developer account.
//
// The patch is needed because the openharmony-rs/emulator-action Oniro emulator enforces app
// code-signature verification but does not trust the public OpenHarmony release cert, so
// `bm install` fails with `code:9568393 verify code signature failed`, for hvigor's signature as
// much as an out-of-band one; the signing method is not the problem. OpenHarmony's BMS, however,
// skips install-time code-sign verification for apps that declare `compileSdkType: "OpenHarmony"`
// on devices that lack Huawei OH code signing (installd `PerformCodeSignatureCheck` returns early
// when `isCompileSdkOpenHarmony && !IsSupportOHCodeSign()`; the Oniro reference build has no
...
//
// The app/profile signature (hapVerify) still runs, so we sign with the SDK's release material
// (which the emulator does accept; installs got past hapVerify to the code-sign stage before).
//
...
// Minimal JSON5 read (strip // and /* */ comments + trailing commas), enough for
// AppScope/app.json5.
