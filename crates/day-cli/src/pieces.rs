...
/// Write the resolved contributions to `build/day/android/day-pieces.json` for Gradle to read.
/// Always writes (an empty manifest when there are no pieces) so a stale file never lingers.
