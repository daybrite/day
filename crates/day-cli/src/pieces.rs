...
pub fn write_ios_pieces(project: &Project) -> Result<Option<String>, String> {
    ...
    // Staged like the macOS leg: only files whose bytes changed are written, and the strays are
    // pruned at the end. xcodebuild's llbuild keys on mtime + size, so rewriting this tree
    // wholesale (as this did before 2026-08) recompiled the whole DayPieces module every build.
    ...
    if !fonts.is_empty() {
        ...
        for f in &fonts {
            ...
            copy_if_changed(&f.path, &fdir.join(name))?;
        }
    }
    // Both trees are written by other generators (write_media_xcassets above, the fonts loop
    // here); collect what they produced so the prune below keeps it.
    collect_files(&sources.join("Media.xcassets"), &mut expected);
...
pub fn write_ohos_pieces(project: &Project, harmony: &Path) -> Result<(), String> {
    ...
    write_if_changed(&contract, DAY_PIECE_ETS)?;
    expected.push(contract);
    for (namespace, src) in &pieces.ets_dirs {
        sync_ets_dir(Path::new(src), &dir.join(namespace), &mut expected)?;
    }
    let aggregator = dir.join("DayPieces.ets");
...
fn sync_ets_dir(
    ...
    for entry in rd.flatten() {
        let path = entry.path();
        if path.is_dir() {
            sync_ets_dir(&path, &dest.join(entry.file_name()), expected)?;
...
/// The binary counterpart of [`write_if_changed`], for fonts and image payloads.
pub(crate) fn write_bytes_if_changed(path: &Path, content: &[u8]) -> Result<(), String> {
    if std::fs::read(path).is_ok_and(|cur| cur == content) {
        return Ok(());
    }
    std::fs::write(path, content).map_err(|e| format!("{}: {e}", path.display()))
}

/// Copy `src` to `dest` only when the bytes differ. `std::fs::copy` always restamps the
/// destination (and does not carry the source mtime), so a plain copy of an unchanged file is
/// enough to invalidate the native build that consumes it.
pub(crate) fn copy_if_changed(src: &Path, dest: &Path) -> Result<(), String> {
    let content = std::fs::read(src).map_err(|e| format!("{}: {e}", src.display()))?;
    write_bytes_if_changed(dest, &content)
}

/// Record every existing file under `root` into `expected`, so [`prune_except`] keeps trees
/// written by another generator (the iOS `Media.xcassets`, the staged `fonts/`) instead of
/// deleting them as strays.
pub(crate) fn collect_files(root: &Path, expected: &mut Vec<std::path::PathBuf>) {
