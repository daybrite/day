            ...
            .filter(|a| a.platforms.iter().any(|p| p == platform))
        {
            match arm.lang {
                        ...
                        && !hms_sdk_available(project) =>
                {
                    ...
                    // out: its Rust half then finds no registered function and answers
...
/// imports it unconditionally and hands its (then empty) record to the native module.
pub fn write_arkts(harmony: &Path, staged: &Staged) -> Result<(), String> {
    ...
    for (i, (file, contents)) in staged.arkts.values().enumerate() {
        ...
        calls.push_str(&format!("  parts.push(register{i}());\n"));
    }
    ...
    let body = format!(
        "// @generated by `day build` — the ArkTS arms of every bridged crate in this app\n\
         ...
         \x20 const parts: Record<string, Function>[] = [];\n\
         {calls}\
         \x20 for (const part of parts) {{\n\
         \x20   for (const key of Object.keys(part)) {{\n\
         \x20     out[key] = part[key];\n\
         \x20   }}\n\
         \x20 }}\n\
         \x20 return out;\n\
         }}\n"
