...
fn stage_icons(project: &Project, prefix: &Path, id: &str) {
    if stage_project_icons(project, prefix, id) == 0 {
        ...
        status(
            "Packing",
            "no app icon rendered — using the default Day icon (add resource/icons/icon.svg and run `day prepare`)",
        );
...
fn stage_project_icons(project: &Project, prefix: &Path, id: &str) -> usize {
    ...
    let entries = std::fs::read_dir(host.join("linux"))
        .or_else(|_| std::fs::read_dir(host.join("png")))
        .or_else(|_| std::fs::read_dir(project.root.join("resource/icons/linux")))
