...
pub fn stage_payload(
    ...
    // packed app would have no vector form at all. The glyph SVGs do not ship
...
pub fn pack(
    ...
    // Generated by `day prepare` (crate::icon), which every build runs first; the legacy
    // export tree is the fallback for a project that has not migrated.
    let logo = [
        project
            .root
            .join(crate::icon::HOST_DIR)
            .join("windows/day-icon-256.png"),
        project.root.join("resource/icons/windows/day-icon-256.png"),
    ]
    .into_iter()
    .find(|p| p.exists())
    .or_else(|| {
    ...
    match logo {
        ...
        None => {
            status(
                "Packing",
                "no app icon rendered — using the default Day icon for the MSIX logo slots (add resource/icons/icon.svg and run `day prepare`)",
            );
...
/// MSIX versions are four-part, so pad the semver.
