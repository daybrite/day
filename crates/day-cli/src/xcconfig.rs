...
pub fn write_generated(project: &Project, platform: &str) -> Result<(), String> {
    ...
    // Day.toml `[window]` is deliberately NOT conveyed here. `INFOPLIST_KEY_*` build settings only
    // apply when Xcode GENERATES the Info.plist, and the scaffold ships a real one
    // (`GENERATE_INFOPLIST_FILE = NO`), so a key written here would be silently dropped. The
    // window keys go through `mobile::sync_window_keys`, which edits the plist itself — the same
    // path the permission usage descriptions take.
    let out = format!(
