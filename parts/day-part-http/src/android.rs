...
// user CA store, plus HTTP/2 and PATCH. The Java call BLOCKS the calling thread (day-android's
// `with_env` attaches ANY thread to the JVM), matching `fetch`'s contract; results cross JNI as
// one byte[] envelope (see DayHttp.java's header). The asynchronous entry points do not come
// through here: they are the crate's bridge arm (`src/bridge.rs`), which runs on OkHttp's own
// dispatcher and completes a token — no thread, and cancellation by that token.
// ---------------------------------------------------------------------------
...
/// A response envelope — from the blocking shim or the bridge arm's completion — as a
/// [`Response`], with the transport sentinels mapped to [`HttpError`].
pub fn response_from_envelope(envelope: &[u8]) -> Result<Response, HttpError> {
