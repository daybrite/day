...
// CMMotionManager reports no sensors on desktop hardware), so everything is unavailable. A
// dedicated stub file keeps the per-OS symmetry (and the test suite green on the mac host).
