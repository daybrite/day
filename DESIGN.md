...
| `day sign check` / `day sign status <id>` / `day sign apply <artifact>` | validate `Day.toml [signing]` without printing secrets, query a notarization submission, or sign a package that already exists (Android; §16.5 below). A subcommand is required |
...
`day sign apply <artifact>` signs a package that already exists, for a distributor that builds and
...
matches an upload against.

Apple follows the same contract with Xcode's export steps written out: embed the profile
(`--profile`, else `signing.ios.profile`), take the entitlements the profile grants unless
`--entitlements` overrides them (what Xcode does, and the only set that installs), sign nested
frameworks and plug-ins deepest-first, sign the bundle with `--generate-entitlement-der`, verify
with `codesign --verify --deep --strict`, repack with `ditto`. `--identity`/`signing.ios.identity`
names the certificate; the flags outrank the manifest because a distributor signs an artifact
whose `Day.toml` names the developer's material, not its own. macOS only (codesign/security/ditto);
`.dmg` and `.pkg` remain `day pack`'s, and are named as not-yet-implemented rather than refused.
...
4. **Per-combo jobs** (macOS: appkit/gtk/qt; Linux: gtk/qt headless; Windows: xaml and an MSYS2
   ...
   >
   > What that buys beyond deduplication: the app pipeline's device matrix. `ios-devices`,
   > `android-devices` and `harmony-devices` take one line per device (a phone and a landscape
   > tablet by default on all three; on HarmonyOS a device is a panel size of the one Oniro
   > image, since the guest draws at whatever QEMU is told), each with its own `os`, and a row marked
...
| `web_eval` | `id`, `script`, `contains?`, `text?`, `timeout_secs?` | evaluate JavaScript in a web-view node and assert on the result ([docs/webview-eval.md](docs/webview-eval.md)) — proves a page RENDERED where `assert_visible` only proves the native view exists; retryable while pending/throwing/mismatching; `timeout_secs` raises the engine and runner wait budgets for cold browser startup or slow page loads (tested by `web_eval_honors_its_own_timeout`); gate with `only_on:` to the backends whose eval arm exists |
