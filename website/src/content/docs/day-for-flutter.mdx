...
**Flutter draws its own widgets; Day doesn't.** Flutter ships a renderer (Impeller/Skia) and
repaints every pixel, so your app looks identical everywhere. Day creates each platform's real
widgets — `NSButton` on macOS, a Material button on Android, GTK, Qt, WinUI, and ArkUI widgets
elsewhere — so your app looks native everywhere instead. Flutter gives you pixel control; Day
gives you platform fidelity. [Why Day](/docs/benefits) lists what each approach costs and when
Flutter is the better pick.
...
incremental and usually seconds, but not sub-second stateful patching, and losing that is real
if you live in it. The replacement loop is `day relaunch` to apply changes, plus
