...
| platform channels | [parts](/docs/parts) — direct FFI, no message codec |
...
through the signal graph. [Reactivity](/docs/reactivity) explains the machinery.
...
There is nothing to dispose: every signal and handler belongs to a scope, and when the page that
created it goes away, the scope tears it all down.
...
The second argument is the key function: the job Flutter's `Key`s do, made mandatory so
...
Flutter hands you the window size and leaves the decision to you: read `MediaQuery.sizeOf`, pick
your own breakpoints, and swap `NavigationRail` for `NavigationBar` yourself. Day buckets the
window into [size classes](/docs/internal/size-classes) — Android's numbers, used verbatim on
every backend — and a `selector` re-presents itself when the window crosses one.
...
The class belongs to a window, not the app, so a second window in a different size gets its own
answer — Stage Manager, Android split-screen, or a desktop window someone is dragging narrower as
they read.
...
chrome (slider tracks, focus rings, checkbox shapes) to the platform on purpose.
[Styling](/docs/styling) lists what you can restyle and what stays native. If your design
system needs every pixel identical on every platform, Flutter fights you less.
...
The Day side has no assets or fonts list: files under `resource/` are found
...
The philosophy is the same; Day moves the configuration out: per-app settings live in
...
Maturity differs by target, and the docs say so rather than implying parity: each `(OS,
toolkit)` pair carries a [support tier](/docs/platforms#support-tiers), from [Tier
...
[dayscript](/docs/dayscript) to script the app to the screen you're iterating on instead of
clicking back to it.
...
- [Why Day](/docs/benefits) — the frank comparison, including when to keep Flutter.
