...
Flutter renders its interface through its graphics engine, giving applications control over
appearance across platforms. Day uses native toolkit controls, such as `NSButton` on macOS
and Material buttons on Android. [Why Day](/docs/benefits) explains the tradeoffs between these
approaches.
