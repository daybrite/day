...
Flutter ships a renderer (Impeller/Skia) and repaints every pixel, so your app looks identical
everywhere. Day creates each platform's real widgets (`NSButton` on macOS, a Material button on
Android, GTK, Qt, system XAML, and ArkUI widgets elsewhere), so your app looks native everywhere
instead. [Why Day](/docs/benefits) lists what each approach costs and when Flutter is the better
pick.
...
or `BuildContext`, and `const` optimization has no equivalent because nothing rebuilds. `Signal<T>`
is `Copy`, so it moves into both closures without a `.clone()`. The function runs once; after that,
updates flow through the signal graph. [Reactivity](/docs/reactivity) explains the signal graph.
...
`NavigationBar` yourself. Day buckets the window into [size classes](/docs/internal/size-classes)
...
The class is per window, so a second window at a different size (Stage Manager, Android
split-screen, a resized desktop window) gets its own answer.
...
Files under `resource/` are found by convention, so the Day side has no assets or fonts list, and
...
embed your own SwiftUI views on the Apple targets ([SwiftUI embedding](/docs/internal/swiftui)).

## Resources and localization

Both frameworks generate typed accessors from your translation files. Flutter uses ARB plus `intl`;
Day uses [Mozilla Fluent](https://projectfluent.org/). Fluent handles plural and gender grammar in
...
Day moves per-app settings into `Day.toml`, so the host scaffolds rarely change. Where Flutter uses
