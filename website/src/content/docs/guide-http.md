...
disposed is a silent no-op — a late completion can't crash a page the user already left.
...
`Signal<Load<T>>`. Unlike `day::task`, the fetcher returns a `Result` — the error becomes
`Load::Failed`:
...
cancel rail as above) and a stale completion writes nothing. Scope disposal aborts it too — no
...
- **The browser adds CORS.** On web, cross-origin requests need the server's opt-in, and
  ...
  spec. Network-level failures surface as `HttpError::Io` — browsers hide DNS/TLS detail.
