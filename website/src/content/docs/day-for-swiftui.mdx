...
| `NavigationStack(path:)` | `stack(path, root)`, driven by a path signal |
...
string. The difference you notice day to day is that you mark what's dynamic yourself: a
closure makes text live, and a bare value doesn't. In exchange, an update's cost is proportional
to the number of readers of the changed value and independent of the size of the view.
...
field is independently observable, and the struct needs no macro or conformance:
...
navigation, state restoration, and deep links with one set of strings, the job `onOpenURL` and
scene restoration do separately. See [Navigation](/docs/navigation).
...
typography scale (on Apple targets, the same text styles you use today). SwiftUI's semantic
colors (`Color.primary`, automatic dark-mode adaptation for your own colors) have no Day
equivalent yet. Native widget chrome follows the system appearance on its own, but hardcoded
custom colors are yours to switch per appearance for now.
...
dependencies. Day splits that into two small text files, both plain text that diffs and merges
cleanly:
...
On macOS, SwiftUI adapts iOS-shaped views, while Day's macOS target is an AppKit app
(`NSButton`, `NSSplitView` source lists, real menu bar items), because Day's macOS toolkit is
built on AppKit.
...
- [Why Day](/docs/benefits) compares the approaches and says when SwiftUI alone is enough.
