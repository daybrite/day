...
chains, semantic fonts, and the same parent-proposes-child-chooses layout protocol. This guide maps
the vocabulary, then covers what changes: the update model (no `body` re-evaluation), the language
(Rust), and the reach (the same codebase also builds for Android, Windows, Linux, HarmonyOS, and the
web with each platform's own widgets).

Every Day renderer is open source. When a control misbehaves on some platform, you can read and
patch the code that placed it. And a move to Day doesn't have to be a rewrite: on Apple targets, Day
[embeds your existing SwiftUI views](#bringing-swiftui-views-with-you) alongside Day-native pages.
...
independent of the size of the view. [Reactivity](/docs/reactivity) explains the model.
...
Day's two navigation Pieces line up with the two SwiftUI containers, down to binding a path
collection for push/pop:
...
`NavigationSplitView` maps to `nav(…).style(NavStyle::Sidebar)`: an `NSSplitView` source list on
macOS, and the platform's own idiom elsewhere (a pushing list on phones, libadwaita's split view on
GTK). On top of the signals sits a thin route layer: `navigate("library/album-42")`,
...
to switch per appearance for now. [Styling](/docs/styling) lists what you can and can't restyle.
...
An Xcode project holds targets, build settings, signing, capabilities, and package dependencies. Day
splits that into two small text files:
...
On iOS, `resource/images/` is staged into an asset catalog and compiled by `actool` (the same
...
catalog. Localization uses [Mozilla Fluent](https://projectfluent.org/) instead of String Catalogs:
...
signal. Switching re-renders live, and RTL mirrors the layout. See [Resources](/docs/resources) and
...
- [Layout](/docs/layout) — the negotiation protocol.
