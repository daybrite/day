...
The generated `install()` registers every locale directory, so a new `resource/locales/<lang>/`
directory is all a new language needs. When you need to assemble the list yourself (locales fetched
at runtime, a per-build subset), the underlying call is public: `day::install_locales("en", &[("en",
…)])` takes any `(tag, source)` list.
...
- **What isn't:** text inside out-of-process UI (native file dialogs, permission prompts) follows
  the *system* locale, not your in-app override; it shows up when you test with `--locale`.
