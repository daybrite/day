...
Day and Compose both track which code reads state, but update the UI differently. Compose
recomposes functions; Day builds the piece tree once and re-runs bindings to update native
widgets. Day uses Material Components on Android and the corresponding toolkit on other platforms.
This guide compares the APIs, state model, packages, and build tools.
