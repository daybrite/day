...
Day's breakpoints are the ones you already know: `SizeClass` uses Android's window size classes
verbatim, on every backend, so a 700pt window is `Medium` on a Mac and in a browser exactly as it
is on a tablet. What differs is who acts on them — in Compose you pick the scaffold per class; in
Day the `selector` re-presents itself.
...
On Android the backend hands the class to `SlidingPaneLayout` and lets it morph natively, with its
own animation and gesture, then reports what it chose — Day observes rather than overrides where
the platform already does this well ([details](/docs/internal/size-classes)). The class is
per window, so split-screen and freeform windows each get their own answer.
