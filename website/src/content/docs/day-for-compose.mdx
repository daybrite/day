...
Day does not use recomposition. The tree is built once, and a state change re-runs only the
closures that read it, each ending in one native setter call. Stability
annotations and `remember` bookkeeping have no equivalent because Day does not re-execute the component function on state changes.

The second difference is widget rendering. Compose draws its own Material widgets on a canvas (and Compose
Multiplatform carries that renderer to other platforms). Day creates each platform's native widgets:
Material Components views on Android, AppKit on macOS, system XAML on Windows, DOM elements on the
web. [Why Day](/docs/benefits) lists what each approach costs and when Compose is the better fit.
...
`setText`. [Reactivity](/docs/reactivity) covers the model and its cost: you mark what's dynamic
...
On Android these are Material Components views (`TextInputLayout`, `MaterialSwitch`), so they track
...
`Font::Title` resolves to each platform's typography scale. Day inherits the platform's look and
ships no design system of its own, so there is no `MaterialTheme` equivalent to recolor control
chrome or to restyle a slider track or checkbox portably. [Styling](/docs/styling) lists what you
can restyle and what stays native, and [tweaks](/docs/tweaks) reach per-platform knobs when one
...
A Gradle project still exists (`platform/android/` is a host that loads the Rust `cdylib`), but
...
own SwiftUI views ([SwiftUI embedding](/docs/internal/swiftui)).
