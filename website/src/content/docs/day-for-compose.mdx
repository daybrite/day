...
Multiplatform carries that renderer to other platforms). Day creates each platform's real widgets:
...
A Gradle project still exists (`platform/android/` is a thin host that loads the Rust `cdylib`), but
