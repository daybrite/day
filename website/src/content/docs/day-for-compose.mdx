...
re-execution. The label's closure is the only code that runs on a tap, and it ends in one
...
[Reactivity](/docs/reactivity) covers the model, including its cost, which is that you mark
what's dynamic yourself (a closure makes text live; a bare value doesn't).

## Two-way controls

Compose controls are stateless value-plus-callback pairs. Day's take signals directly, with
the write-back built in.
...
There are two differences. The key is a required argument, where Compose's `key` lambda is
optional. And `list` recycles real native cells (a `RecyclerView` on Android), so the row
builder runs per recycled slot rather than per composition.
...
`SizeClass` uses Android's window size classes verbatim, on every backend, so a 700pt window
is `Medium` on a Mac and in a browser exactly as it is on a tablet. In Compose you pick the
scaffold per class; in Day the `nav` re-presents itself.
...
On Android the backend hands the class to `SlidingPaneLayout`, which morphs with its own
animation and gesture and then reports what it chose; Day follows the platform's choice where
the platform already makes one ([details](/docs/internal/size-classes)). The class is per
...
measured by the platform's own engine. Day's equivalent of `Modifier.weight(1f)` is `.grow()`,
...
`Font::Title` resolves to each platform's typography scale. Day inherits the platform's look
and ships no design system of its own, which sets a limit: there is no `MaterialTheme`
equivalent to recolor control chrome or to restyle a slider track or checkbox portably.
[Styling](/docs/styling) lists what you can restyle and what stays native, and
...
project reads them generically. Adding such a package to `Cargo.toml` completes the
...
key or wrong argument count is a compile error, and a plural nav host is typed numeric. On
...
- [Why Day](/docs/benefits) compares the approaches and says when Compose fits better.
