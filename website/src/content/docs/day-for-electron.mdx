...
Electron and Day solve the same problem — one codebase, every desktop — with opposite materials.
...
The shapes rhyme — a container, a text node, a click handler — but the right side is not a
...
The three-file round trip on the left is the boundary tax: every capability needs a handler, a
bridge entry, and a serializable payload. On the right the dialog is `open_file()`, the battery
is `day_part_battery::status()`, the clipboard is `day_part_clipboard::get_text()` — plain
functions, typed end to end, with nothing marshalled through strings. When a capability needs
...
## Real widgets, not a rendered page

Form controls bind two ways out of the box, and each one is the platform's own control — which
is also where accessibility, keyboard traversal, right-to-left mirroring, and the user's text
...
`.corner_radius`, semantic `Font::Title`), and [Styling](/docs/styling) is explicit about the
line Day draws: you adjust spacing, color, and typography, and the controls themselves stay
native rather than becoming a themeable canvas. If the design brief is a fully bespoke skin on
every pixel, a web renderer serves that better; if the brief is "a real Mac app on the Mac", the
line is the point.
...
The security model shrinks with the payload. There is no `nodeIntegration` checklist because
...
elements — the demo-without-installing story Electron apps usually rebuild separately. For a
...
- **Identical pixels everywhere.** Your app will look like a Mac app on macOS and a GTK app on
  GNOME. That is the trade, not a bug — but if your brand requires one rendering on every OS,
