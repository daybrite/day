...
React Native and Day both render the platform's widgets: an RN `<Button>` is a `UIButton`, and
...
each platform's networking stack ([`day-part-http`](/docs/parts)), so system proxies, VPN
...
Both projects carry `android/` and `ios/` directories. Day's hosts are host projects that load
