...
(`Slider` lives in `@react-native-community/slider`, outside RN core. Day ships sliders,
toggles, progress, pickers, and a recycling list as built-ins.)

## Lists

`FlatList` maps to `list`: both virtualize, and both want a key. Day's rows recycle real
native cells (`UITableView`-style), and the key function is a required argument, where
`keyExtractor` is optional, so reconciliation is always by identity.
...
The same route strings serve navigation, the `DAY_DEEPLINK` launch variable, and
[dayscript](/docs/dayscript) test assertions, so deep links need no separate `linking` config.
...
backend), and a `selector` re-presents itself when the window crosses one, on a tablet, in
split-screen, or in a resized desktop window.
...
The class is per window, so an app showing two windows at different sizes gets a separate
answer for each.

## Styling and layout

RN styles are flexbox via Yoga. Day also owns its layout engine; it uses a proposal protocol
(parent proposes, child chooses), closer to SwiftUI than to CSS. The daily vocabulary
translates directly:
...
each platform's own networking stack ([`day-part-http`](/docs/parts)), so system proxies, VPN
...
Cargo resolves and builds everything, including packages with native halves, in one step. A
package that needs platform code, the Turbo Module case, is a [part](/docs/parts) (headless
capability) or a [piece](/docs/extending) (a widget): it declares its Swift, Java, or C++ in
...
Both projects carry `android/` and `ios/` directories. Day's hosts are thin shells that load
...
TypeScript. You ship one compiled binary per platform. One repository covers mobile, desktop,
and web, UI tests run the same script everywhere, and `day pack` produces signed artifacts
directly. Signing secrets are `${ENV_VAR}` references in `Day.toml`; a missing one degrades to
...
- [Why Day](/docs/benefits) compares the approaches and says when React Native fits better.
