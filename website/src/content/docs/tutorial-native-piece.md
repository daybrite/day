...
description: Build a piece backed by a REAL native control on each platform — the front-end in Rust, plus per-toolkit backends (objc2 for AppKit/UIKit, gtk-rs, a Qt C++ shim, an Android Java factory, WinUI) — registered with zero edits to any core Day crate.
order: 32
---

Some Pieces cannot be composed from `label`, `button`, and `stack`. A map, an embedded web view, a
native search field with its magnifier and clear button — these are *real platform controls* with
input handling, assistive-technology behavior, and pixels that only the toolkit can produce. To ship
one as a first-class Day Piece you write a small cross-platform front-end in Rust and then a **native
backend for each toolkit you want to support**.
...
Android Java factory, once as a WinUI C++/WinRT shim. That is genuinely more work than a composite
piece. The payoff is a control that is indistinguishable from a built-in `text_field` on every
platform, installable by any app with a single line in `Cargo.toml`, and requiring **zero edits to
any core Day crate**.
...
Ask one question: **is there a real native control I am wrapping, or am I arranging Pieces?**
...
| Wrapping one real native widget per platform | **Native piece** (this tutorial) | A front-end **plus** a backend per toolkit. |
...
behavior for free that a hand-rolled `text_field` would not.

Set expectations honestly: you will implement `make`/`update`/`measure` once per toolkit, and each
...
one `src/lib-<backend>.rs` renderer per toolkit, the `[features]` table, and — where a backend needs
native glue — the C++ shim + `build.rs` (Qt/WinUI), the Java shim + `[package.metadata.day.android]`
(Android/`widget`), and the `[package.metadata.day.ios]` block (iOS/`uikit`):
...
- **A backend per toolkit** — one file each (`lib-appkit.rs`, `lib-gtk.rs`, `lib-qt.rs`,
  ...
  turns the `KIND` leaf into a real native widget.
...
enum, no match arm, no registry table. Merely linking your crate into the AppKit build inserts its
renderer.
...
`UISearchTextField`, attach a per-node target on `UIControlEvents::EditingChanged` that emits
`Event::TextChanged`, and — since programmatic `setText:` does not fire `EditingChanged` — no echo
...
node id; `textChanged` calls back with a UTF-8 string; programmatic `setText` is wrapped in
...
`cc` (MSVC) + the Windows SDK cppwinrt projection. It is Windows-only, built in CI, and — like most
teams — you will likely not verify it locally.
...
Day does not fail silently. The backend logs a **loud, once-per-kind warning** and renders a visible
placeholder in its place (`warn_missing_renderer` in each toolkit crate); in debug builds the §8.2
registration check panics first. So a half-finished piece degrades noisily instead of vanishing.

## 6. The LLM workflow

Here is the honest part. Writing the same widget in Objective-C, C++, Java, and C++/WinRT is the
tedious core of a native piece — and it is exactly the kind of narrow, well-specified translation that
**LLMs are genuinely good at.** Lean into that.

The trick is to make the boundary crisp *first*, then delegate each backend body:
...
3. **Freeze the flat FFI/JNI signatures.** For the toolkits that need a native asset, decide the C ABI
   ...
   signatures (`makeSearch(long, String, String) -> View` / `setSearchText(View, String)`) *now*. They
   are the exact spec you hand the model.
...
compiles is a body whose `Props`/`Patch` handling is type-checked — the model cannot silently mismatch
the protocol. Second, an **un-written backend degrades to the loud placeholder** rather than breaking
...
registration check tell you honestly which ones are still stubs.
