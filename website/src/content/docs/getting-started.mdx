...
Pick whichever method fits your machine — each one ends with a `day` binary on your PATH:

<InstallPicker compact />

The shell and PowerShell one-liners download a prebuilt binary for your platform from the latest
GitHub release, verify its checksum, and install it into `~/.cargo/bin` (or the first writable
equivalent; set `DAY_INSTALL_DIR` to choose). The Homebrew command installs the same prebuilt
binary through `brew`. The Cargo route compiles from source — slower, but works anywhere Rust
does. Day isn't on crates.io or in a Homebrew tap yet, which is why the commands point at the
repository and release assets directly. Whichever you pick, check it worked:

```bash
day --version
```

To build apps you also need a recent stable Rust via rustup (the toolchain requirement moves with the repo's MSRV;
