...
build`, `cargo test`, and `cargo clippy` work in it without the `day` CLI — plus a `Day.toml`
...
Add a target with `day app add-toolkit <target>` (or scaffold with several `--toolkit` flags up front), then launch
